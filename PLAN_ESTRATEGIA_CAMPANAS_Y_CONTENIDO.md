...
```mermaid
flowchart LR
    A[📢 Meta Ads\nReels Dra. Noelia + Feed] --> C[📝 Artículo de Blog\nBeneficios Médicos Sueroterapia]
    B[🔍 Google Search Ads\n'sueroterapia gualeguaychú/buenos aires'] --> C
    C --> D[💬 Botón Flotante WhatsApp\n'Quiero reservar mi turno para el 19/09']
    D --> E[👩‍💼 Laura / Ceci\nConfirmación de Turno en Agenda]
```
...
```mermaid
graph TD
    subgraph NIVEL1["1. Métricas de Pauta (Meta / TikTok / Google)"]
        CPM["CPM (Costo por mil impresiones)"]
        CTR["CTR (> 1.8% en Meta, > 3% en Google)"]
        CPC["CPC (Costo por clic)"]
    end

    subgraph NIVEL2["2. Métricas de Conversión (Leads & Tráfico)"]
        CPL["CPL (Costo por conversación iniciada en WhatsApp)"]
        CPA_Shop["CPA Tienda (Costo por compra de crema en /tienda)"]
        VisitasBlog["Visitas y tiempo de lectura en artículo del Blog"]
    end

    subgraph NIVEL3["3. Métricas de Negocio (Consultorio Real)"]
        TurnosAsistidos["Turnos Efectivamente Realizados en Consultorio"]
        CAC["CAC (Costo de Adquisición de Paciente Nuevo)"]
